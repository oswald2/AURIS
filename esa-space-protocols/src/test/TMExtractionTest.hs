...
pusPacketEncoding _cfg = do
    let pusPkt = PUSPacket pusHdr' pusDfh' Nothing (HexBytes payload) True
        pusHdr' =
    ...
    -- T.putStrLn $ "Encoded PUS Packet:\n" <> hexdumpBS encPusPkt
    -- T.putStrLn $ "Decoded PUS Packet:\n" <> T.pack (show decodedPusPkt)
...
pusPacketExtraction cfg = do
    ...
    let pusPkt = PUSPacket pusHdr' pusDfh' Nothing (HexBytes payload) True
        ...
        conduit =
            C.sourceList frames .| tmFrameExtraction (IfNctrs 1) .| C.consume
    ...
    let storeFrame =
        ...
        extractedPacket = ExtractedDU
            ...
            , _epDU      = PUSPacket
                               ...
                               , _pusPIs       = Nothing
                               , _pusData      = HexBytes "\NUL\NUL\NUL\NUL"
                               , _pusEncodeCRC = True
                               }
            }

        conduit =
            C.sourceList [storeFrame]
...
missingFrame cfg = do
    ...
    let pusPkt = PUSPacket pusHdr' pusDfh' Nothing (HexBytes payload) True
        ...
        frames'' = zipWith (makeTMFrame (Just (packValues defaultCLCW)))
                           hdrs
                           frameData
