...
module Graphics.Rendering.Chart.Backend.GI.Cairo
    ( FileFormat(..)
    , FileOptions(..)
    , runBackend
    , renderableToFile
...
defaultEnv alignFns = CEnv { ceAlignmentFns = alignFns
...
runBackend' env m = eval env (view m)
  where
    ...
    eval env (GetAlignments :>>= f) =
        return (ceAlignmentFns env) >>= step env f
    eval env (WithTransform m p :>>= f) = cWithTransform env m p >>= step env f
    eval env (WithFontStyle font p :>>= f) =
        cWithFontStyle env font p >>= step env f
    eval env (WithFillStyle fs p :>>= f) =
        cWithFillStyle env fs p >>= step env f
    eval env (WithLineStyle ls p :>>= f) =
        cWithLineStyle env ls p >>= step env f
    eval env (WithClipRegion r p :>>= f) =
        cWithClipRegion env r p >>= step env f
...
walkPath (ArcNeg p r a1 a2 path) =
    C.arcNegative (p_x p) (p_y p) r a1 a2 >> walkPath path
walkPath End   = return ()
...
cTextSize text = do
    ...
    fe <- C.fontExtents
    return $ TextSize { textSizeWidth    = C.textExtentsWidth te
...
setSourceColor c =
    let (RGB r g b) = toSRGB $ colourChannel c
