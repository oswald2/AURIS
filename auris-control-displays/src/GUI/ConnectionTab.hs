...
createConnectionTab config builder = do
    ...
    ndiu  <- getObject builder "ndiuConnBox" Box

    let setupFrame :: Text -> IO Frame
        setupFrame name = do
...
-- create a box for the content
        ...
        convNctrs hm cfg = do
            ...
            n1    <- newConnectionStatus id1
                                         "TC"
                                         (cfgNctrsHost cfg)
                                         (cfgNctrsPortTC cfg)
            n2 <- newConnectionStatus id1
                                      "TM"
                                      (cfgNctrsHost cfg)
                                      (cfgNctrsPortTM cfg)
        ...
        convNdiu :: ConnMap -> NDIULiteConfig -> IO ConnMap
        convNdiu hm cfg = do
            let (id1, txt) = (IfNdiu (cfgNdiuID cfg), "NDIU")

            frame <- setupFrame (cfgNdiuName cfg)
            box   <- setupBox
            ...
            boxPackStart ndiu frame False False 5
    ...
    conns3 <- foldlM convNdiu conns2 (cfgNDIU config)
    conns  <- foldlM convCnC conns3 (cfgCnC config)

    let g = ConnectionTab { _connTabEntries  = conns
                          , _connTabNctrsBox = nctrs
                          , _connTabCncBox   = cnc
                          , _connTabEdenBox  = eden
                          , _connTabNdiuBox  = ndiu
                          }
