...
instance GetValue Word8 where
    getValue bytes (ByteOffset idx) _ =
        if idx >= B.length bytes then Nothing else Just $ bytes `B.index` idx
    {-# INLINABLE getValue #-}

instance GetValue Int8 where
    getValue bytes (ByteOffset idx) _ = if idx >= B.length bytes
        then Nothing
        else Just . fromIntegral $ bytes `B.index` idx
    {-# INLINABLE getValue #-}
        ...
        let !val = case byteOrder of
                ...
                LiE ->
                    fromIntegral (bytes `B.index` idx + 1)
                        `shiftL` 8
                        .|.      fromIntegral (bytes `B.index` idx)
        in  if idx + 1 >= B.length bytes then Nothing else Just val
    {-# INLINABLE getValue #-}
...
instance GetValue Word24 where
    getValue bytes (ByteOffset idx) BiE =
        let b0   = fromIntegral (bytes `B.index` (idx + 3)) `shiftL` 16
            ...
            !val = b0 .|. b1 .|. b2
        in  if idx + 2 >= B.length bytes then Nothing else Just val
    getValue bytes (ByteOffset idx) LiE =
        let b0   = fromIntegral (bytes `B.index` idx)
            b1   = fromIntegral (bytes `B.index` (idx + 1)) `shiftL` 8
            b2   = fromIntegral (bytes `B.index` (idx + 2)) `shiftL` 16
        ...
        let val   = getWord48Val <$> getValue @Word48 bytes off endian
            !val2 = fmap
                (\val' ->
                    Int48
                        . fromIntegral
                        $ if val' .&. 0x00_00_80_00_00_00_00_00 /= 0
                              then val' .|. 0xFF_FF_00_00_00_00_00_00
                              else val'
                )
                val
        in  val2
...
instance GetValue Int24 where
    getValue bytes off endian =
        let val   = getValue @Word24 bytes off endian
            !val2 = fmap
                (\val' -> Int24 . fromIntegral $ if val' .&. 0x00_80_00_00 /= 0
                    then val' .|. 0xFF_00_00_00
                    else val'
                )
                val
        in  val2
...
instance GetValue Int64 where
    getValue bytes off endian =
        fromIntegral <$> getValue @Word64 bytes off endian
    {-# INLINABLE getValue #-}
...
instance GetValue MILSingle where
    getValue bytes off endian =
        MILSingle . decMILSingle <$> getValue @Word32 bytes off endian
...
instance GetValue MILExtended where
    getValue bytes off endian =
        MILExtended
            .   decMILExtended endian
            .   getWord48Val
            <$> getValue @Word48 bytes off endian
...
byteSwap48 (Word48 w) =
    let b0   = w .&. 0xFF `shiftL` 40
        ...
        !out = b5 .|. b4 .|. b3 .|. b2 .|. b1 .|. b0 `shiftL` 40
    in  Word48 out

{-# INLINABLE signExtend48 #-}
signExtend48 :: Word48 -> Int48
signExtend48 (Word48 val) =
    let !val2 = if val .&. 0x00_00_80_00_00_00_00_00 /= 0
            then val .|. 0xFF_FF_00_00_00_00_00_00
            else val
...
getBitFieldInt64 bytes off bits BiE =
    signExtend bits <$> getBitField bytes off bits
getBitFieldInt64 bytes off bits LiE =
    let val = getBitField bytes off bits
    in
        if
            | bits == 64 -> fromIntegral . byteSwap64 <$> val
            | bits == 32 -> fromIntegral . byteSwap32 . fromIntegral <$> val
            | bits == 16 -> fromIntegral . byteSwap16 . fromIntegral <$> val
            | bits == 48 ->  fromIntegral
            .   signExtend48
            .   byteSwap48
            .   fromIntegral
            <$> val
...
getBitFieldDouble bytes off BiE =
    wordToDouble <$> getBitField bytes off (BitSize 64)
getBitFieldDouble bytes off LiE =
    wordToDouble . byteSwap64 <$> getBitField bytes off (BitSize 64)

{-# INLINABLE getBitFieldFloat #-}
getBitFieldFloat :: ByteString -> Offset -> Endian -> Maybe Float
getBitFieldFloat bytes off BiE =
    wordToFloat . fromIntegral <$> getBitField bytes off (BitSize 32)
getBitFieldFloat bytes off LiE =
    wordToFloat . byteSwap32 . fromIntegral <$> getBitField bytes
                                                            off
                                                            (BitSize 32)

{-# INLINABLE getBitFieldMilSingle #-}
getBitFieldMilSingle :: ByteString -> Offset -> Endian -> Maybe Double
getBitFieldMilSingle bytes off BiE =
    decMILSingle . fromIntegral <$> getBitField bytes off (BitSize 32)
getBitFieldMilSingle bytes off LiE =
    decMILSingle . byteSwap32 . fromIntegral <$> getBitField bytes
                                                             off
                                                             (BitSize 32)

{-# INLINABLE getBitFieldMilExtended #-}
getBitFieldMilExtended :: ByteString -> Offset -> Endian -> Maybe Double
getBitFieldMilExtended bytes off endian =
    decMILExtended endian <$> getBitField bytes off (BitSize 48)
...
getBitField bytes off bs@(BitSize nBits) =
    let
        ...
        loop1 !nb !val !ix
            | nb >= 8
            = let !val2 =
                      (val `shiftL` 8) .|. fromIntegral (bytes `B.index` ix)
    ...
    in
        if (unByteOffset . fst . offsetParts . nextByteAligned) (off .+. bs)
                > B.length bytes
            then Nothing
            else Just value2
...
decMILSingle b =
    let
        ...
        !expd = fromIntegral exp'' * if signExponent == 0 then 1 :: Int else -1
