...
instance SetValue Word24 where
    setValue vec off' BiE val = do
        let !val1 = fromIntegral $ (val `shiftR` 16) .&. 0xFF
            !val2 = fromIntegral $ (val `shiftR` 8) .&. 0xFF
            !val3 = fromIntegral $ (val .&. 0xFF)
            off   = unByteOffset off'
        VS.unsafeWrite vec off val1
        VS.unsafeWrite vec (off + 1) val2
        VS.unsafeWrite vec (off + 2) val3
    setValue vec off' LiE val = do
        let !val3 = fromIntegral $ (val `shiftR` 16) .&. 0xFF
            !val2 = fromIntegral $ (val `shiftR` 8) .&. 0xFF
            !val1 = fromIntegral $ (val .&. 0xFF)
            off   = unByteOffset off'
        VS.unsafeWrite vec off val1
...
instance SetValue Int24 where
    setValue vec off endian val =
        setValue vec off endian (fromIntegral val :: Word24)
    {-# INLINABLE setValue #-}
